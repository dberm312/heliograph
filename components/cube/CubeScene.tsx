...
// Face 0 (front) → Face 3 (left) → Face 2 (back) → Face 1 (right) → repeat
// This maps faceIndex to its position in that sequence
const FACE_ORDER: Record<number, number> = { 0: 0, 1: 3, 2: 2, 3: 1 };
...
export function CubeScene() {
  ...
  // Phase 1: Panels appear (0-15%)
  const panelOpacity = [
    interpolate(effectiveProgress, [0, 0.05], [0, 1]),
    interpolate(effectiveProgress, [0.03, 0.08], [0, 1]),
    interpolate(effectiveProgress, [0.06, 0.11], [0, 1]),
  ];

  // Phase 2: Fold into cube (15-35%)
  const foldProgress = interpolate(effectiveProgress, [0.15, 0.35], [0, 1]);

  // Phase 3: Spin 3 rotations (35-80%)
  const spinProgress = interpolate(effectiveProgress, [0.35, 0.8], [0, 1]);
  const spinRotationY = spinProgress * 1080; // 3 full rotations
  ...
  const rotationQuarter = Math.floor(spinRotationY / 90);
  ...
  // Left face: starts at x=-320, folds to left side
  ...
  // Right face: starts at x=320, folds to right side
  ...
  const isCubeFormed = effectiveProgress >= 0.35;
